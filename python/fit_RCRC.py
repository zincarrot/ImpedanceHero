# -*- coding: utf-8 -*-
"""
Created on Sun Oct 10 20:09:19 2021
...
def sconnect(Z1,Z2,s):
    return (Z1**s+Z2**s)**(1/s)
...
def imp_relax(f,para):
    ...
    c = para[6]
    jj = 0+1j
    val=sconnect(sconnect(R1,ZC(C1,f),a),sconnect(R2,ZC(C2,f),b),c)
    return val

bounds_min =[1, -18, 3, -8, -1, -1, 0]
bounds_max =[3, -10, 9, -4,  0,  0, 1]
...
z_error = []
r2 = []
x_init=np.array([2, -14,  7,  -6,  -0.9, -0.3, 1])
...
for it in range(len(T_list)):
    ...
    for i in range(N):
        ...
        for j in obj:
            opt = minimize(j, x_init, method='powell', bounds=bounds, options={'maxiter': 500,'disp':False})
        ...
        scale=(obj_logzMSE_all(x_opt))*x_opt*np.array([5,5,5,5,5,5,5])*0.1
    ...
    for i in range(N):
        ...
        scale=(obj_logzMSE_all(x_opt))*x_opt*np.array([5,5,5,5,5,5,5])*0.05
        x_init=x_opt+np.random.random(size=len(x_opt))*scale-0.5*scale
        # plt.scatter((1 / (1j * 2 * np.pi * freq_list * imp_comp)).real,
        #             (1 / (1j * 2 * np.pi * freq_list * imp_comp)).imag)
        # plt.scatter((1 / (1j * 2 * np.pi * freq_list * (imp_relax(freq_list, x_init)))).real,
